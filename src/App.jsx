...
function Contact(){
  const reasons=[
    {title:'Ask an implementation question',text:'Describe the operating condition, system boundary, or decision you are trying to understand.',subject:'OSI implementation question'},
    {title:'Suggest a correction or source',text:'Identify material that may need clarification and include the authoritative source that supports the change.',subject:'OSI correction or source suggestion'},
    {title:'Discuss standards collaboration',text:'Start a conversation about research, working groups, interoperability, or standards development.',subject:'OSI standards collaboration'},
    {title:'Contribute professional insight',text:'Share a recurring condition, evidence pattern, or industry perspective that could strengthen OSI guidance.',subject:'Contributing professional insight to OSI'},
    {title:'Request permission or clarification',text:'Ask about reuse, attribution, educational application, or the intended meaning of OSI material.',subject:'OSI permission or clarification'},
    {title:'Report a website or tool problem',text:'Tell us which page or download you used, what you expected, and what happened.',subject:'OSI website or tool issue'}
  ]
  return <section className="section contact" data-page="contact">
    <div className="section-number">17 / CONTACT</div>
    <div className="contact-intro"><h2>Questions improve<br/>the <em>work.</em></h2><div><p>OSI welcomes informed questions, corrections, collaboration, and practical insight from people responsible for security, technology, compliance, operations, procurement, and continuity.</p><p>Choose the reason that best fits your message. The prepared link will open an email with the subject already entered.</p></div></div>
    <div className="contact-grid">{reasons.map((reason,index)=><article key={reason.title}><span>{String(index+1).padStart(2,'0')}</span><h3>{reason.title}</h3><p>{reason.text}</p><a className="text-link" href={`mailto:info@outputsecurityinstitute.org?subject=${encodeURIComponent(reason.subject)}`}>Email OSI <ArrowRight size={15}/></a></article>)}</div>
    <div className="contact-direct"><div><small>DIRECT CONTACT</small><h3>info@outputsecurityinstitute.org</h3></div><a className="button primary" href="mailto:info@outputsecurityinstitute.org?subject=Output%20Security%20Institute%20inquiry">Write to OSI <ArrowRight size={17}/></a></div>
    <div className="contact-limit"><ShieldCheck size={34}/><div><small>IMPORTANT BOUNDARY</small><h3>Contact begins a conversation, not a compliance relationship.</h3><p>OSI provides independent educational guidance. Contact does not create an advisory, legal, certification, attestation, or compliance relationship. Do not send sensitive, regulated, proprietary, or controlled information by email.</p></div></div>
  </section>
}
...
function Header(){
  ...
  const links=[['Start Here','/profiles.html'],['OICC Controls','/controls.html'],['Standards','/standards.html'],['Industries','/industries.html'],['Evidence','/evidence.html'],['Resources','/resources.html'],['Tools','/tools.html'],['Contact','/contact.html']]
...
function App(){
  ...
  <section className="hero" data-page="home">
    ...
    <p className="hero-copy">Printers, scanners, workflows, cloud connections, service access, and the information they handle form an operational system. OSI helps organizations understand that system, identify hidden dependencies, and decide what to examine first.</p>
    <p className="hero-reassurance">You do not need to begin with a framework. Begin with the condition you are trying to understand.</p>
    <div className="hero-actions"><a className="button primary" href="/profiles.html">Help me get started <ArrowRight size={18}/></a><a className="text-link" href="/controls.html">Explore the OICC controls <ArrowRight size={18}/></a></div>
  ...
  <section className="section first-questions" data-page="home">
    <div className="section-number">BEGIN WITH ONE QUESTION</div>
    <div className="first-question-intro"><h2>You do not need to understand<br/>everything to <em>begin.</em></h2><p>Choose the question closest to the condition in front of you. Each one leads to a practical starting point, not another wall of terminology.</p></div>
    <div className="first-question-grid"><a href="/profiles.html"><span>01</span><h3>What is part of our output system?</h3><p>Define the devices, information, people, applications, connections, and support paths that belong inside the boundary.</p><b>Find the starting condition <ArrowRight size={15}/></b></a><a href="/profiles.html"><span>02</span><h3>Where do we depend on outside access or support?</h3><p>Make external service, remote access, parts, expertise, response time, and recovery assumptions visible.</p><b>Examine the dependency <ArrowRight size={15}/></b></a><a href="/evidence.html"><span>03</span><h3>What would prove that we are in control?</h3><p>Identify the records, settings, observations, tests, and ownership decisions needed to support the conclusion.</p><b>Explore evidence patterns <ArrowRight size={15}/></b></a></div>
  </section>
